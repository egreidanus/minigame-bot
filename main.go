...
import (
	...
	"github.com/egreidanus/minigamebot/minigamebot/commands"
	"github.com/egreidanus/minigamebot/minigamebot/components"
	"github.com/egreidanus/minigamebot/minigamebot/handlers"
)
...
func main() {
	...
	cfg, err := minigamebot.LoadConfig(*path)
	if err != nil {
	...
	setupLogger(cfg.Log)
	slog.Info("Starting minigamebot...", slog.String("version", Version), slog.String("commit", Commit))
	slog.Info("Syncing commands", slog.Bool("sync", *shouldSyncCommands))

	b := minigamebot.New(*cfg, Version, Commit)
...
func setupLogger(cfg minigamebot.LogConfig) {
